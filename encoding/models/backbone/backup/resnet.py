...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, dilated=True, norm_layer=nn.BatchNorm2d, multi_grid=False, multi_dilation=None,os=8,deep_base=True):
        #self.inplanes = 64
        ...
        if dilated:
            if multi_grid:
                if os == 16:
                    ...
                    #                           multi_grid=multi_grid, multi_dilation=multi_dilation[:1]) 
        ...
        if multi_grid:
            ...
            for i in range(1,blocks):
                ...
                layers.append(block(self.inplanes, planes, dilation=multi_dilation[i%div], previous_dilation=dilation,
                                                    norm_layer=norm_layer))
               # else:
               #     layers.append(block(self.inplanes, planes, dilation=4, previous_dilation=dilation,
               #                                     norm_layer=norm_layer))
        else:
            for i in range(1, blocks):
